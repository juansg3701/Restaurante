...
class menuController extends Controller
{
 	...
 	public function store(UserFormRequest $request){
 		...
 		if($correo==auth()->user()->email &&  password_verify($contraseña, auth()->user()->password)){
 			$id=auth()->user()->id;
	 		$cliente=Cliente::findOrFail($id);
	 		$users=Users::findOrFail($id);

	 		$pedido=DB::table('pedido')
	 		->where('cliente_id_cliente','=',$cliente->id_cliente)
	 		->orderBy('id_pedido', 'desc')
	 		->get();
	 		...
	 		if(count($pedido)>0){
	 			...
	 			foreach ($pedido as $p) {
	 				...
	 				$pe=Pedido::findOrFail($p->id_pedido);
	 				$pe->cliente_id_cliente=1;
	 				$pe->estado=1;
	 				$pe->save();
	 			}
	 		}
 	...
 	public function show($id){
	 	...
	 	public function destroy($id){
	 		...
	 		return back()->with('msj','Pedido cancelado');
	 	}
	 	...
	 	public function edit($id){
	 		...
	 		return back()->with('msj','Pedido solicitado');
	 	}
}
